...
<div class="w-full space-y-8 animate-fade-in">
  <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
    <div>
      ...
      <p class="text-[11px] text-gray-400 font-medium">
        Quản lý điểm lấy hàng và tra cứu hệ thống bưu cục toàn quốc
      </p>
    </div>
    <div class="flex bg-gray-100 p-1.5 rounded-[22px] w-fit shadow-inner">
      <button
        onclick="switchTab('my-addresses')"
  ...
  <div
    ...
    <button
      ...
      <span
        class="text-[10px] font-black uppercase tracking-widest text-gray-400 mt-4 group-hover:text-[#00B14F]"
        >Thêm kho hàng mới</span
      >
    </button>
    ...
    <c:forEach items="${addresses}" var="addr">
      <div
        ...
        <div class="flex items-center justify-between relative z-10">
          <span
            class="px-3 py-1 bg-green-50 text-[#00B14F] text-[9px] font-black uppercase rounded-lg border border-green-100"
            >Cơ sở lấy hàng</span
          >
          <a
        ...
        <div class="mt-8 relative z-10">
          ...
          <p class="text-xs font-bold text-gray-400 mt-1">
            <i class="fa-solid fa-phone mr-2 text-[10px]"></i
            >${addr.contactPhone}
          </p>
          <p
            class="text-[11px] text-gray-500 mt-4 leading-relaxed line-clamp-2">
            ${addr.addressDetail}
          </p>
        </div>
        <i
      ...
      <div
        ...
        <div class="relative flex-shrink-0">
          ...
          <span
            ...
            ${hub.province == 'Ho Chi Minh' || hub.province == 'Ha Noi' ?
            'Central Hub' : 'Branch'}
          </span>
        </div>
        ...
        <div class="flex-1 space-y-4 text-center md:text-left">
          <div>
            <h4
              class="text-lg font-black text-gray-800 uppercase tracking-tight">
              ${hub.hubName}
            </h4>
            <div
              class="flex flex-wrap justify-center md:justify-start gap-2 mt-2">
              <span
                class="px-2 py-0.5 bg-gray-50 text-gray-400 text-[9px] font-bold rounded-md border border-gray-100"
                >Phường ${hub.ward}</span
              >
              <span
                class="px-2 py-0.5 bg-gray-50 text-gray-400 text-[9px] font-bold rounded-md border border-gray-100"
                >${hub.district}</span
          ...
          <p class="text-[11px] text-gray-500 leading-relaxed font-medium">
            <i class="fa-solid fa-location-arrow mr-2 text-orange-400"></i
            >${hub.address}
          </p>
          ...
          <div
            ...
            <div class="flex items-center space-x-4">
              <span class="text-[10px] font-black text-gray-400"
                ><i class="fa-solid fa-clock mr-2 text-green-500"></i>08:00 -
                21:00</span
              >
            </div>
            <button
...
<div
  ...
  <div
    ...
    <form
      action="/customer/addressbook/add"
      method="POST"
      class="p-10 space-y-6">
      <h3
        class="text-sm font-black text-gray-800 uppercase tracking-widest text-center">
        Thêm địa chỉ kho hàng
      </h3>
      <div class="space-y-4">
        <div>
          <label class="text-[10px] font-black text-gray-400 uppercase ml-2"
            >Tên người liên hệ/Kho</label
          >
          <input
            type="text"
            name="contactName"
      ...
      <div class="flex space-x-3 pt-4">
        <button
          type="button"
          onclick="closeModal()"
          class="flex-1 py-3 text-[10px] font-black uppercase text-gray-400">
          Hủy bỏ
        </button>
        <button
          type="submit"
          class="flex-[2] bg-gray-800 text-white py-3 rounded-2xl text-[10px] font-black uppercase hover:bg-black transition-all">
          Lưu địa chỉ
        </button>
      </div>
    </form>
...
<script>
  ...
  function openMapModal(hubName, address, ward, district, province) {
    ...
    var fullAddress =
      address + ", " + ward + ", " + district + ", " + province + ", Vietnam";
  ...
  window.onclick = function (event) {
    ...
    if (event.target == addrModal) closeModal();
