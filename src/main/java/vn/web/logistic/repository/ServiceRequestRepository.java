...
public interface ServiceRequestRepository extends JpaRepository<ServiceRequest, Long> {
    ...
    // Đơn cần PICKUP: status = pending, currentHub = hub hiện tại của manager
    ...
    List<ServiceRequest> findPendingPickupByHubId(@Param("hubId") Long hubId);
    ...
    // Đơn cần PICKUP với phân trang: status = pending, currentHub = hub của manager
    @Query(value = "SELECT r FROM ServiceRequest r " +
            ...
            "ORDER BY r.createdAt DESC", countQuery = "SELECT COUNT(r) FROM ServiceRequest r " +
                    ...
                    +
                    "AND t.taskStatus IN ('assigned', 'in_progress'))")
    Page<ServiceRequest> findPendingPickupByHubIdPaged(
            @Param("hubId") Long hubId,
            Pageable pageable);
    ...
    // Đếm tổng đơn cần PICKUP: status = pending, currentHub = hub của manager
    @Query("SELECT COUNT(r) FROM ServiceRequest r " +
            "WHERE r.status = 'pending' " +
            "AND r.currentHub.hubId = :hubId " +
            "AND NOT EXISTS (SELECT t FROM ShipperTask t WHERE t.request = r AND t.taskType = 'pickup' " +
            "AND t.taskStatus IN ('assigned', 'in_progress'))")
    long countPendingPickupByHubId(@Param("hubId") Long hubId);
    ...
    @Query("SELECT function('DATE', s.createdAt) as date, COUNT(s) as totalOrders, COALESCE(SUM(s.codAmount), 0) as totalCod "
            +
            "FROM ServiceRequest s " +
            "WHERE s.customer.customerId = :customerId AND s.createdAt >= :startDate " +
            "GROUP BY function('DATE', s.createdAt) " +
    ...
    List<ServiceRequest> findByCustomerAndStatusInOrderByCreatedAtDesc(Customer customer,
            List<RequestStatus> statuses);

    List<ServiceRequest> findByCustomerOrderByCreatedAtDesc(Customer customer);

    List<ServiceRequest> findByCustomerAndStatusAndPaymentStatus(Customer customer, RequestStatus status,
