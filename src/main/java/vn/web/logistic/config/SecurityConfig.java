...
public class SecurityConfig {
        ...
        public SecurityFilterChain apiFilterChain(HttpSecurity http) throws Exception {
                http
                                ...
                                // Nếu để STATELESS thì SecurityContext không đọc từ HttpSession => các page sẽ
                                // bị 401 khi fetch /api/**
        ...
        public SecurityFilterChain webFilterChain(HttpSecurity http) throws Exception {
                http
                                ...
                                .securityMatcher("/**")
                                .csrf(csrf -> csrf.disable()) // Tắt CSRF (trong thực tế Web nên bật cái này, nhưng demo
                                                              // thì tắt cho dễ)
                                ...
                                .authorizeHttpRequests(auth -> auth
                                                ...
                                                // Các trang Auth - Login, Register, Forgot Password
                                                .requestMatchers("/auth/**").permitAll()

                                                // Các trang Public (Legacy routes - redirect)
                                                ...
                                                // Phân quyền truy cập trang theo 4 Role: ADMIN, STAFF, SHIPPER,
                                                // CUSTOMER
                                ...
                                .formLogin(form -> form
                                                .loginPage("/auth/login") // URL trang login custom của bạn
                                                .loginProcessingUrl("/do-login") // URL action trong thẻ <form> để
                                                                                 // submit username/password
                                                .usernameParameter("identifier") // Tên field input trong form (email
                                                                                 // hoặc phone)
                                                .passwordParameter("password") // Tên field input password trong form
                                                .defaultSuccessUrl("/login-success", true) // Chuyển hướng sau khi login
                                                                                           // thành công -> redirect
                                                                                           // theo role
                                                .failureUrl("/auth/login?error=true") // Chuyển hướng khi login sai
                                                .permitAll())

                                // Cấu hình Đăng xuất
                                .logout(logout -> logout
                                                .logoutUrl("/logout") // URL để gọi logout (đơn giản hơn)
                                                .logoutSuccessUrl("/auth/login?logout=true") // Chuyển hướng sau khi
                                                                                             // logout
                                                .invalidateHttpSession(true) // Hủy session
                                                .deleteCookies("JSESSIONID", "JWT_TOKEN") // Xóa cả JWT cookie
                                                .permitAll())
